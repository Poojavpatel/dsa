const wordSearch = (board, word) => {
  ...
  const findWord = (i, j, letterToFindIndex) => {
    ...
    if(board[i][j] == "-" || board[i][j] !== lettersToFind[letterToFindIndex]) return false;
    if(letterToFindIndex == lettersToFind.length - 1) return true;
    const temp = board[i][j];

    board[i][j] = '-';

    const found = findWord(i+1, j, letterToFindIndex + 1)
...
console.log(wordSearch([["C","A","A"],["A","A","A"],["B","C","D"]], "AAB"))
